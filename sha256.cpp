...
{
    if(!ctx)
        ctx = reinterpret_cast<SHA256_CTX*>(malloc(sizeof(SHA256_CTX)));
...
QString SHA256Hash(QString str)
{
    SHA256_CTX *ctx = NULL;
    unsigned char hash[32];
    QString buf;
    unsigned char *string = reinterpret_cast<unsigned char*>(const_cast<char*>(str.toStdString().c_str()));

    sha256_init(ctx);
    sha256_update(ctx, string, str.length());
    sha256_final(ctx, hash);

    for (int i=0; i < 32; i++)
        buf += printfify("%02x", hash[i]);
